...
hooks: enabled: - "crash_monitor" output:
...
扫描结果保存在 results/ 目录下的 summary_.json 文件中。可以使用以下命令快速查看统计：
...
def close_hook(ql, fd, retval): if retval == -9 and fd > 1024: return 0 return retval ql.os.set_syscall('close', close_hook, QL_INTERCEPT.EXIT)。
